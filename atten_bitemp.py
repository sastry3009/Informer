...
class CABiTempConv(Disaggregator):

    def __init__(self, params):

        self.MODEL_NAME = "Channel Attn Bi directional Temporal Conv"
        ...
        self.nb_filters = params.get('nb_filters',128)
    ...
    def partial_fit(self,train_main,train_appliances,do_preprocessing=True,**load_kwargs):
        ...
        print("...............CABTCN partial_fit running...............")
        ...
        for appliance_name, power in train_appliances:
            ...
            if train_main.size > 0:
                ...
                if len(train_main) > 10:
                    ...
                    #filepath = 'cabitcn-temp-weights-'+str(random.randint(0,100000))+'.h5'
                    #checkpoint = ModelCheckpoint(filepath,monitor='val_loss',verbose=1,save_best_only=True,mode='min')
                    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir="./cabt")
    ...
    def SqueezeExcite(self, x, ratio=2):
    	nb_chan = K.int_shape(x)[-1]
	
    	y = GlobalAveragePooling1D()(x)
    	y = Dense(nb_chan // ratio)(y)
    	y = Activation('relu')(y)
    	y = Dense(nb_chan)(y)
    ...
    def residual_block(self, x, dilation_rate, nb_filters, kernel_size, padding, activation='relu', dropout_rate=0, kernel_initializer='he_normal'):
                   ...
                   for k in range(2):
                   	...
                   	#att_x = self.SqueezeExcite(x, ratio=2)
                   	res_x = add([prev_x, x])
                   	#####################################
    ...
    def quantile_loss(self, q, y, y_p):
        err = y-y_p
        delta = 1.0
        huber_loss = tf.losses.huber(y, y_p, delta)
        quantile_loss = tf.reduce_mean(tf.maximum(q * (y - y_p), (1 - q) * (y_p - y)))
        total_loss = tf.reduce_mean(huber_loss + quantile_loss)
        return total_loss
    ...
    def return_network(self):
        ...
        with strategy.scope():
        	...
        	x =  self.SqueezeExcite(x, ratio=2)
        	...
        	model.summary()
        	quantile = 0.65
        	#lambda y, y_p: self.quantile_loss(quantile, y, y_p)
        	model.compile(optimizer='adam', loss=lambda y, y_p: self.quantile_loss(quantile, y, y_p))
